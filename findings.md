...
## 2026-04-30 8位userId生成设计

### 背景
SnowflakeIdGenerator.nextId() 返回 64 位 long，传到前端页面导致整数溢出。需要生成 8 位数字用于页面展示。

### 方案选择
- **选择方案 B**：时间戳后6位 + 随机2位
- 原因：展示用途不需要保证严格唯一性，时间戳提供趋势递增感
...
### 输出示例
- 时间戳 `123456` + 随机 `78` → `12345678`
- 时间戳 `1` + 随机 `23` → `00000123`
